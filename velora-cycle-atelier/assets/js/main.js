...
(function ($) {
    ...
    $(document).on('click', 'a[href^="#"]', function (e) {
        ...
        $('.at-offcanvas').removeClass('opened');
        $('.body-overlay').removeClass('apply');
        $('.at-offcanvas-2-area').removeClass('menu-open menu-open-temp');
        $('.hamburger-open-btn, .hamburger-close-btn').removeClass('active');

        const header = document.querySelector('.velora-site-header');
        const headerHeight = header ? header.getBoundingClientRect().height : 0;
        const fallbackOffset = window.innerWidth < 768 ? 136 : 112;
        const offset = Math.max(headerHeight + 18, fallbackOffset);
        ...
        if (smoother) {
            smoother.scrollTo(Math.max(top, 0), true);
            return;
        }
    ...
    ////////////////////////////////////////////////////
    // 12.y Hide/reveal landing header
    (function () {
        const header = document.querySelector('.velora-site-header');
        if (!header) {
            return;
        ...
        const topThreshold = 36;
        const deltaThreshold = 8;

        header.classList.add('velora-header-visible');

        const updateHeaderHeight = () => {
            document.documentElement.style.setProperty('--velora-header-height', `${Math.ceil(header.getBoundingClientRect().height)}px`);
        };

        const setHeaderVisible = (isVisible) => {
            header.classList.toggle('velora-header-hidden', !isVisible);
            header.classList.toggle('velora-header-visible', isVisible);
        };
